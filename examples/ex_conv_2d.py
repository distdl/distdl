...
global_tensor_shape = np.array([1, 1, 10, 10])

layer = DistributedConv2d(P_cart, in_channels=1, out_channels=1, kernel_size=[3, 3])

x = NoneTensor()
if P_cart.active:
    input_tensor_sizes = compute_subsizes(P_cart.dims, P_cart.cartesian_coordinates(P_cart.rank), global_tensor_shape)
