def test_conv_3d_no_bias_parallel():
    ...
    P = P_world.create_partition_inclusive(np.arange(12))
    P_cart = P.create_cartesian_topology_partition([1, 1, 3, 2, 2])

    global_tensor_sizes = np.array([1, 5, 10, 10, 10])

    layer = DistributedConv3d(global_tensor_sizes, P_cart,
...
def test_conv_3d_bias_only_parallel():
