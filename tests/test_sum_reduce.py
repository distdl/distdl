...
def test_sum_reduce_parallel_overlap():
    ...
    layer = SumReduce(P_in, P_out, out_root=out_root)
    ...
    Ax = SumReduceFunction.forward(ctx, x_clone,
                                   ...
                                   layer.out_root, layer.dtype)

    # Apply A*
    Asy = SumReduceFunction.backward(ctx, y_clone)[0]
...
def test_sum_reduce_sequential():
    ...
    # Apply A*
    Asy = SumReduceFunction.backward(ctx, y.clone())[0]
