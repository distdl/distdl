...
adjoint_parametrizations.append(
    pytest.param(
        ...
        np.arange(0, 6), [2, 3, 1, 1],  # P_w_ranks, P_w_shape
...
adjoint_parametrizations.append(
    pytest.param(
        ...
        np.arange(0, 3), [1, 3, 1, 1],  # P_y_ranks, P_y_shape
        np.arange(0, 6), [3, 2, 1, 1],  # P_w_ranks, P_w_shape
        [1, 8, 10, 10],  # x_global_shape
        6,  # passed to comm_split_fixture, required MPI ranks
...
adjoint_parametrizations.append(
    pytest.param(
        np.arange(0, 2), [1, 2, 1, 1],  # P_x_ranks, P_x_shape
        np.arange(0, 1), [1, 1, 1, 1],  # P_y_ranks, P_y_shape
        np.arange(0, 2), [1, 2, 1, 1],  # P_w_ranks, P_w_shape
        [1, 8, 10, 10],  # x_global_shape
...
adjoint_parametrizations.append(
    pytest.param(
        ...
        np.arange(0, 2), [1, 2, 1, 1],  # P_y_ranks, P_y_shape
        np.arange(0, 2), [2, 1, 1, 1],  # P_w_ranks, P_w_shape
        [1, 8, 10, 10],  # x_global_shape
        2,  # passed to comm_split_fixture, required MPI ranks
        id="distributed-co3_ci2",
        marks=[pytest.mark.mpi(min_size=2)]
...
def test_channel_conv2d_adjoint_input(barrier_fence_fixture,
                                      comm_split_fixture,
                                      P_x_ranks, P_x_shape,
                                      P_y_ranks, P_y_shape,
                                      P_w_ranks, P_w_shape,
    ...
    layer = DistributedChannelConv2d(P_x, P_y, P_w,
...
@pytest.mark.parametrize("P_x_ranks, P_x_shape,"
                         ...
                         adjoint_parametrizations,
                         indirect=["comm_split_fixture"])
def test_channel_conv2d_adjoint_weight(barrier_fence_fixture,
                                       comm_split_fixture,
                                       P_x_ranks, P_x_shape,
                                       P_y_ranks, P_y_shape,
                                       P_w_ranks, P_w_shape,
...
@pytest.mark.parametrize("P_x_ranks, P_x_shape,"
                         ...
                         adjoint_parametrizations,
                         indirect=["comm_split_fixture"])
def test_channel_conv2d_adjoint_bias(barrier_fence_fixture,
                                     comm_split_fixture,
                                     P_x_ranks, P_x_shape,
                                     P_y_ranks, P_y_shape,
                                     P_w_ranks, P_w_shape,
